...
int MRD(double h1, double h2, double **a, double **b, double **F, double **w, double **r, double **Ar, 
    ...
    do {
        ...
        if(obj_i - 1 >= 0){
            ...
            MPI_Request send_up;
            MPI_Isend(top_send_buf, col_end - col_start - 1, MPI_DOUBLE, obl_index_to_rank(obj_i - 1, obj_j, n, k), msg_iter, MPI_COMM_WORLD, &send_up);
        ...
        if(obj_i + 1 < n){
            ...
            MPI_Isend(down_send_buf, col_end - col_start - 1, MPI_DOUBLE, obl_index_to_rank(obj_i + 1, obj_j, n, k), msg_iter, MPI_COMM_WORLD, &send_down);
